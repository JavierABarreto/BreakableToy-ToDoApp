...
export const ToDoTable = ({ todos, setType }) => {
  ...
  const sorterHandlerPriority = useCallback(() => {
    setDateOrder("default")
    dispatch(setFilter({ payload: dOrder, type: "sortByDate" }))
    setPriorityOrder(pOrder == "asc" ? "dsc" : "asc")
    
    sorter("priority")
  }, [dispatch, pOrder, dOrder, setPriorityOrder, setDateOrder, sorter])

  const sorterHandlerDate = useCallback(() => {
